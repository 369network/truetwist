...
import { getGrowthTacticRecommendations } from '@/lib/recommendations';
import { PlatformSchema } from '@/lib/social/types';

// GET /api/v1/recommendations/growth-tactics?socialAccountId=...&platform=...
export async function GET(request: NextRequest) {
  try {
    ...
    const recommendation = await getGrowthTacticRecommendations(
      user.sub,
      socialAccountId,
      parsedPlatform
    );
