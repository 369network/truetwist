...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 gradient-hero relative overflow-hidden" style={{ background: "var(--tt-bg)" }}>
      {/* Background orbs */}
      <div className="absolute top-10 right-1/3 w-72 h-72 rounded-full animate-glow-orb" style={{ background: "radial-gradient(circle, rgba(168,85,247,0.15), transparent 70%)" }} />
      <div className="absolute bottom-10 left-1/4 w-64 h-64 rounded-full animate-glow-orb" style={{ background: "radial-gradient(circle, rgba(255,107,107,0.1), transparent 70%)", animationDelay: "1.5s" }} />
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold" style={{ fontFamily: "var(--font-heading)" }}>Create your account</h1>
          <p className="text-sm mt-1" style={{ color: "var(--tt-text-muted)" }}>Start creating viral content with AI</p>
        </div>
        <form onSubmit={handleSignup} className="p-8 rounded-2xl space-y-5 glow" style={{ background: "var(--tt-surface)", border: "1px solid var(--tt-border)" }}>
          ...
          <div className="relative my-4">
            <div className="absolute inset-0 flex items-center"><div className="w-full" style={{ borderTop: "1px solid var(--tt-border)" }} /></div>
            <div className="relative flex justify-center"><span className="px-3 text-xs" style={{ background: "var(--tt-surface)", color: "var(--tt-text-muted)" }}>or continue with</span></div>
          </div>
          <button type="button" className="w-full py-2.5 rounded-xl text-sm font-medium transition hover:-translate-y-0.5 flex items-center justify-center gap-2" style={{ background: "var(--tt-surface-2)", border: "1px solid var(--tt-border)", color: "var(--tt-text)" }}>
